...
def table_html(rows, headers=None, max_rows=60):
    ...
    colgroup = ''.join(f"<col style='min-width:{'280px' if col_width_class(h)=='wide' else '150px' if col_width_class(h)=='mid' else '92px'};width:{'280px' if col_width_class(h)=='wide' else '150px' if col_width_class(h)=='mid' else '92px'}'>" for h in headers)
...
def build_top_cards(top_rows, entry_rows, guide_rows, news_rows):
    ...
    for r in top_rows[:15]:
        ...
        cards += f"""<article class="card"><h3>#{esc(rank)} {esc(name)}</h3><div class="meta">{esc(normalize_category(name, sector))} · 현재가 {esc(price)} · 실전점수 {esc(score)}</div><p>{esc(basic)}</p><p><span class="pill">{esc(entry_decision or '진입판정 확인')}</span></p><p><b>공격/기준/보수:</b> {esc(pick(e, ['공격진입가']))} / {esc(pick(e, ['기준진입가']))} / {esc(pick(e, ['보수진입가']))}</p><p><b>돌파/손절:</b> {esc(pick(e, ['돌파진입가']))} / {esc(pick(e, ['손절기준가']))}</p><p><b>익절/손절 계획:</b> {esc(pick(r, ['익절계획']) or pick(g, ['take_profit_guide']))} / {esc(pick(r, ['손절계획']) or pick(g, ['stop_loss_guide']))}</p></article>"""
