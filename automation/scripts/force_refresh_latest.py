...
def _pct_values(text):
    import re
    return [v.replace(' ', '') for v in re.findall(r"\+?\d+(?:\.\d+)?\s*%", str(text or ''))]

def fmt_tp(text):
    vals = _pct_values(text)
    if len(vals) >= 3:
        return f"<b>{esc(vals[0])}(60)</b> / <b>{esc(vals[1])}(20)</b> / <b>{esc(vals[2])}(20)</b>"
    if len(vals) == 2:
        return f"<b>{esc(vals[0])}(60)</b> / <b>{esc(vals[1])}(40)</b>"
    if len(vals) == 1:
        return f"<b>{esc(vals[0])}</b>"
    return esc(text)
...
def dashboard_validation_section():
    return "<section class='box minor'><h2>추천후보 대시보드 + 검증</h2><p class='hint'>추천후보 상세의 대시보드와 전략검증을 한 페이지로 묶었습니다. 핵심 화면에서는 작게 두고 필요할 때 확인하면 됩니다.</p><a class='biglink' href='../details/legacy_candidate_dashboard_validation.html'>대시보드/검증 열기</a></section>"
...
def main():
    ...
    html_text=unified_html(stamp,ss); Path('docs/mobile/index.html').write_text(html_text,encoding='utf-8'); Path('docs/latest/index.html').write_text("<!doctype html><html lang='ko'><head><meta charset='utf-8'><meta http-equiv='refresh' content='0; url=../mobile/'><meta name='viewport' content='width=device-width,initial-scale=1'><title>통합 홈으로 이동</title></head><body><p><a href='../mobile/'>통합 홈으로 이동</a></p></body></html>", encoding='utf-8')
    Path('docs/data/latest_publish_status.csv').write_text(f'key,value\npublished_at,{stamp}\nsession,{ss}\nsource,single_home_v12_2_14\n',encoding='utf-8-sig')
