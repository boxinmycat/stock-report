...
def session():
    env = (
        os.environ.get("REPORT_SESSION") or os.environ.get("SESSION") or ""
    ).strip().upper()
    if env in ("AM", "PM", "PM_TEST", "MANUAL"):
        return env
...
def fmt_tp(text):
    ...
    # 🧼 기호 누락 무력화: 익절 순서쌍 정밀 강제 정렬 (+3%(60%) 포맷)
    if len(vals) >= 6:
...
def fmt_sl(text):
    t = str(text or "").strip()
    vals = re.findall(r"\d+(?:\.\d+)?", t)
    # 🧼 기호 누락 무력화: 손절 비율 및 분할 비중 자동 짝맞춤 교정 (-2%(80%) 포맷)
    if len(vals) >= 6:
        return (
...
def top15_entry_section():
    ...
    for r in rows[:6]:
        ...
        <article class='card'>
            ...
            <p style="font-size:12px; margin:2px 0;">🎯 <b>익절계획:</b> {fmt_tp(r.get('take_profit_plan') or r.get('익절계획'))}</p>
            <p style="font-size:12px; margin:2px 0;">🚨 <b>손절계획:</b> {fmt_sl(r.get('stop_loss_plan') or r.get('손절계획') or r.get('stop_price'))}</p>
        </article>"""
...
def holdings_section():
    ...
    desc = {
        r.get("stock_name"): r
        for r in read_csv("docs/data/latest_holding_stock_descriptions.csv", 50)
    }
    if not rows:
        return "<section class='box'><h2>보유종목 현황</h2><p class='hint'>보유 종목 내역이 없습니다.</p></section>"
    cards = ""
    for r in rows[:8]:
...
def ai_section():
    ...
    if not rows:
        return "<section class='box'><h2>🧠 제미나이 AI 보유 브리핑</h2><p class='hint'>장마감 리포트에서 분석이 반영됩니다.</p></section>"
    cards = ""
    for r in rows[:5]:
        summary_raw = (
            r.get("ai_issue_summary") or r.get("ai_three_line_summary") or ""
        )
...
</head><body><main class='wrap'><section class='hero'><h1>📱 모바일 관제 대시보드</h1><p>갱신: {esc(stamp)} | 세션: {esc(ss)}</p></section><section class='nav-grid'><a href='../details/legacy_top15.html'>TOP15 리서치</a><a href='../details/legacy_continuous.html'>🔄 연속추천 관찰</a><a href='../v11_holdings/'>보유종목 상세</a><a href='../details/holding_ai_briefing.html'>🧠 AI 자산 브리핑</a><a href='../details/naver_news.html'>📰 뉴스 3줄요약</a><a href='../details/trade_evaluation.html' style='background:#0284c7; color:white;'>📈 AI 매매기록 평가</a><a href='../downloads/' style='grid-column: span 2; background:#1e3a8a; color:white;'>📥 전체 엑셀 센터 열기</a></section>
    {top15_entry_section()}
    {holdings_section()}
    {ai_section()}
    {news_section()}
...
def main():
    ...
    Path("docs/latest/index.html").write_text(
        "<!doctype html><html lang='ko'><head><meta charset='utf-8'><meta http-equiv='refresh' content='0; url=../mobile/'><meta name='viewport' content='width=device-width,initial-scale=1'></head><body></body></html>",
        encoding="utf-8",
    )
    print(
        "✅ 에러 없는 모바일 전용 480px 강력 압축 홈 대시보드 및 포맷 버그 완전 가동."
    )
