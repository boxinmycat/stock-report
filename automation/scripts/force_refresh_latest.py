...
def top15_entry_section():
    ...
    if not rows: return "<section class='box accent'><h2>추천 TOP15 + 진입 시나리오</h2><p class='hint'>데이터 수집 확인 필요</p></section>"
    cards = ''
    for r in rows[:6]:
        cards += f"""
        <article class='card'>
            <h3>{esc(r.get('rank'))}. {esc(r.get('stock_name'))} <span class='pill'>{esc(r.get('score'))}점</span></h3>
            <p class='meta'>{esc(r.get('sector'))} · 현재가 {esc(r.get('current_price'))} · <b>{esc(r.get('entry_decision'))}</b></p>
...
def ai_section():
    ...
    cards = ''
    for r in rows[:5]:
        summary = r.get('ai_three_line_summary') or ''; action = r.get('ai_action_headline') or ''
        cards += f"""
        <article class='card' style='border-top:3px solid #1e3a8a;'>
            ...
            <p class='desc' style='font-size:12px;'>{str(summary).replace('\\n','<br>')}</p>
        </article>"""
...
def main():
    ...
    print('✅ 모바일 전용 480px 강력 압축 홈 대시보드 및 포맷 버그 제어 빌드 완료.')
