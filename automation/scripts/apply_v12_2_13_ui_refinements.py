...
def write_csv(rows, path):
    ...
    if not rows:
        rows = [{"message": "데이터 없음", "checked_at": now()}]
    fields = []
    for r in rows:
        for k in r:
            if k not in fields:
...
def is_etf(name):
    ...
    up = (name or "").upper()
    return any(b.upper() in up for b in brands)


def theme(name):
    n = name or ""
    rules = [
        ("미국 S&P500", ["S&P500", "S＆P500", "미국S&P"]),
...
def desc(name, category):
    if is_etf(name):
        t = theme(name)
        return f"{name}은(는) {t} 컨셉의 ETF입니다. {ETF_HINTS.get(t, ETF_HINTS['ETF'])}"
    if name in COMPANY_HINTS:
        return COMPANY_HINTS[name]
    c = category or ""
    if "자동차" in c:
        return f"{name}은(는) 자동차 부품·소재 밸류체인과 연결되는 기업입니다. 완성차 생산, 수주, 원가와 환율 흐름을 함께 봅니다."
    if any(x in c for x in ["철강", "소재", "화학"]):
        return f"{name}은(는) 소재·화학 업황과 연결되는 기업입니다. 원재료 가격, 전방 산업 수요, 수익성 회복 여부가 중요합니다."
    if "반도체" in c:
        return f"{name}은(는) 반도체 밸류체인과 연결되는 종목입니다. 업황 회복, 장비·소재 투자, 고객사 수요를 함께 봅니다."
    if "바이오" in c:
        return f"{name}은(는) 바이오·헬스케어 흐름과 연결됩니다. 임상, 허가, 기술이전, 실적 가시성을 함께 확인해야 합니다."
...
def fmt_sl(text):
    t = str(text or "").strip()
    vals = re.findall(r"\d+(?:\.\d+)?", t)
    # 🧼 기호 누락 무력화: 손절 값과 분할 비중 짝맞춤 자동 교정
    if len(vals) >= 6:
        return (
            f"<span style='color:#dc2626; font-weight:bold;'>-{vals[0]}%({vals[3]}%)</span> / "
    ...
    if len(vals) >= 4:
        return (
            f"<span style='color:#dc2626; font-weight:bold;'>-{vals[0]}%({vals[2]}%)</span> / "
            f"<span style='color:#dc2626; font-weight:bold;'>-{vals[1]}%({vals[3]}%)</span>"
        )
    if len(vals) == 3:
        return (
            f"<span style='color:#dc2626; font-weight:bold;'>-{vals[0]}%(80%)</span> / "
...
def rebuild_top15():
    ...
    cards = ""
    for r in rows[:15]:
        name = r.get("stock_name") or r.get("종목명")
        cat = r.get("sector") or r.get("섹터/분야")
        tp = fmt_tp(
            r.get("take_profit_plan")
            or r.get("익절계획")
            or r.get("take_profit_guide")
        )
        sl = fmt_sl(
            r.get("stop_loss_plan")
            or r.get("손절계획")
            or r.get("stop_loss_guide")
            or r.get("stop_price")
        )
    ...
    html_text = f"<!doctype html><html lang='ko'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>추천 TOP15 + 진입 시나리오</title><style>{css}</style></head><body><main class='wrap'><section class='hero'><h1>추천 TOP15 + 진입 시나리오</h1><p>추천 후보를 처음 보는 salmon도 어떤 기업/ETF인지 먼저 이해할 수 있도록 기본 정보를 정리했습니다.</p></section>{cards}</main></body></html>"
    Path("docs/details/legacy_top15.html").write_text(
        html_text, encoding="utf-8"
    )
    Path("docs/details/recommendation_top15.html").write_text(
        html_text, encoding="utf-8"
    )
...
def restyle(path):
    ...
    txt = txt.replace("min-width:900px", "min-width:1250px").replace(
        "width:100%;", "width:max-content;min-width:100%;"
    )
    txt = txt.replace(
        "</style>",
        "td:nth-child(n+8){min-width:380px;max-width:560px;}td:nth-child(-n+7){min-width:95px;max-width:180px;}td{word-break:keep-all;white-space:normal;}</style>",
    )
...
def news_page():
    ...
    recent = [
        r
        for r in rows
        if "May" not in (r.get("pubDate") or "")
        and " 5월 " not in (r.get("title") or "")
    ]
    use = (recent or rows)[:20]
    summary = read_csv("docs/data/latest_major_news_summary.csv", 1)
    s = (
        summary[0].get("summary") if summary else "최신 뉴스 요약 데이터 확인 필요"
    )
    items = ""
    for r in use:
        link = r.get("link") or "#"
        items += f"<article class='card'><div class='meta'>{esc(r.get('query'))} · {esc(r.get('pubDate'))}</div><h2>{esc(r.get('title'))}</h2><p>{esc(r.get('description'))}</p><a href='{esc(link)}' target='_blank' rel='noopener'>기사 보기</a></article>"
    ...
    Path("docs/details/naver_news.html").write_text(
        f"<!doctype html><html lang='ko'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>주요 뉴스 요약</title><style>{css}</style></head><body><main class='wrap'><section class='hero'><h1>주요 뉴스 요약</h1><p>오래된 기사 fallback을 줄이고, 최근 기사 중심으로 정리합니다.</p></section><section class='summary'>{esc(s)}</section>{items}</main></body></html>",
        encoding="utf-8",
    )
...
def main():
    rebuild_top15()
    restyle("docs/details/legacy_full_recommendations.html")
    restyle("docs/details/recommendation_full_list.html")
    restyle("docs/details/legacy_candidate_dashboard_validation.html")
    news_page()
