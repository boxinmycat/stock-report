...
def call_gemini(prompt):
    ...
    for model in models:
        try:
            ...
            payload = {
                "system_instruction": {"parts": [{"text": "당신은 한국 주식 투자자를 위한 리서치 노트 작성자입니다. 출력은 반드시 미니 리서치 노트여야 합니다. 이미 표에 있는 현재가, 목표가, 손절가, 실전점수, 진입판정, 수익률 숫자를 본문에서 반복하지 마세요. 흔한 업종 개요와 무의미한 수식어를 금지하고, 고객사/수주/실적/공시/ETF 노출 테마 강도처럼 투자 판단에 필요한 정보만 씁니다. 반드시 JSON만 출력하세요."}]},
...
요구:
- 절대 금지: 현재가, 목표가, 손절가, 실전점수, 진입판정, 수익률 숫자를 본문에서 반복하지 마세요.
- 절대 금지: '업황에 따라 변동', '성장성이 기대', '주의가 필요'처럼 누구에게나 붙는 흔한 문장을 쓰지 마세요.
- 반드시 1. 분석 컨셉, 2. 한 줄 요약, 3. 핵심 투자 포인트, 4. 주의 리스크 구조로 작성하세요.
- 핵심 투자 포인트에는 고객사/수주/실적/공시/전방산업 변화 또는 ETF 노출 테마 강도를 넣으세요.
- ETF는 어떤 자산군·산업·테마에 노출되는지, 액티브/테마형 리스크를 설명하세요.
- 개별 종목은 무엇을 만들고 파는지, 고객사/전방산업/실적·공시·수급 포인트를 설명하세요.
